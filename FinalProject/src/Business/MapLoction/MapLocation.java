...
public class MapLocation extends MapView {
    ...
    public MapLocation(EcoSystem system,String id,boolean sentFlag) {
        this.system=system;
        this.id=id;
    ...
    public void addNotify() {
        ...
        optionsWindow = new OptionsWindow(this, new Dimension(350, 40)) {
            ...
            public void initContent(JWindow contentWindow) {
                ...
                final JButton sendLocationButton = new JButton();
            ...
            //    searchButton.setIcon(new ImageIcon(MapOptionsExample.class.getResource("res/search.png")));
              //  searchButton.setRolloverIcon(new ImageIcon(MapOptionsExample.class.getResource("res/search_hover.png")));
                ...
                ActionListener searchActionListener = new ActionListener() {
                  ...
                  ActionListener sendLocationListener = new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        performLocCode(searchField.getText());
                    }
                };
                searchButton.addActionListener(searchActionListener);
                searchField.addActionListener(searchActionListener);
                sendLocationButton.addActionListener(sendLocationListener);
                ...
                content.add(sendLocationButton, new GridBagConstraints(1, 0, 1, 1, 0.0, 0.0,
  ...
  private void performLocCode(String text) {
        // Getting the associated map object
        final Map map = getMap();
        // Creating a geocode request
        GeocoderRequest request = new GeocoderRequest();
                            ...
                            for(Organization org :((HospitalEnterprise) enterprise).getOrganizationDirectory().getOrganizationList())
                            {
                                if(org instanceof PatientOrganization)
                                {
                                    for(UserAccount userAccount:((PatientOrganization)org).getUserAccountDirectory().getUserAccountList())
                                    {
                                        ...
                                        {
                                            userAccount.getPatient().getEmergencyContact().setLocation(result.getFormattedAddress());
                                            System.out.println(result.getFormattedAddress());
                                            EmailService obj = new EmailService();
                                            obj.mailSend("vasudevan.n@husky.neu.edu","Emergency required for person !!" +userAccount.getPatient().getFirstName() + "admiited at " + result.getFormattedAddress());
                                            sentFlag = true;
